    ...
    if cb_data.startswith("ytdata||"):
        ...
        if media_type == 'audio':
            buttons = InlineKeyboardMarkup([[InlineKeyboardButton(
                "دریافت صدا", callback_data=f"{media_type}||{format_id}||{yturl}"), InlineKeyboardButton("دریافت داکیومنت",
                                                                                                    callback_data=f"docaudio||{format_id}||{yturl}")]])
        else:
            buttons = InlineKeyboardMarkup([[InlineKeyboardButton(
                "دریافت ویدیو", callback_data=f"{media_type}||{format_id}||{yturl}"), InlineKeyboardButton("دریافت داکیومنت",
...
async def catch_youtube_dldata(c, q):
    ...
    await q.edit_message_reply_markup(
        InlineKeyboardMarkup([[InlineKeyboardButton("در حال دانلود...", callback_data="down")]]))
...
async def send_file(c, q, med, filename):
    ...
    try:
        await q.edit_message_reply_markup(
            InlineKeyboardMarkup([[InlineKeyboardButton("در حال آپلود...", callback_data="down")]]))
        ...
        try:
          thumbnail_url = "https://telegra.ph/file/ce37f8203e1903feed544.png"
            await q.edit_message_reply_photo(thumbnail_url, caption=title, reply_markup=buttons)  
